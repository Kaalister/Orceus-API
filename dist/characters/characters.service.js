...
let CharactersService = class CharactersService {
    ...
    createCharacter(newCharacter) {
        return this.charactersRepository.save(this.charactersRepository.create(newCharacter));
    }
    async updateCharacter(id, updatedCharacter) {
        const character = await this.charactersRepository.findOneBy({ id });
        if (!character)
            throw new common_1.NotFoundException(`The character with the id : ${id} not found`);
        return await this.charactersRepository.save(this.charactersRepository.create({
            ...character,
            ...updatedCharacter,
        }));
    }
